...
import { ASN1, ASN1MsgUtils, EncodingRule, sanitize } from '../src';
...
describe('Kerberos Protocol', () => {
    it('test hex base64 conv', () => {
        const base64Msg = readFileSync(resolve(__dirname, 'data/kerberos/krb-ap-req.base64.der'), {
            encoding: 'utf8',
        });
        const hexMsg = readFileSync(resolve(__dirname, 'data/kerberos/krb-ap-req.hex.der'), {
            encoding: 'utf8',
        });
    ...
    it('test kerberos AP-REQ msg', () => {
        const output = ASN1Node.parseFileMsg(resolve(__dirname, 'data/kerberos/krb-ap-req.hex.der'), {
            encodingRule: EncodingRule.DER,
        });
        assert.deepStrictEqual(output, kerberosJson);
    });
    it('test kerberos asn.1 file from the RFC4120', () => {
        const definition = readFileSync(resolve(__dirname, 'data/kerberos/kerberos.asn1'), {
            encoding: 'utf8',
        });
        const module = ASN1.getModuleFromStr(definition);
        // console.log('module: ', inspect(module, false, null, true));
        const validated = ASN1.validate(module, kerberosJson, 'GSS-API');
    ...
    it('get the PrincipalName', () => {
        const generalStrings = ASN1MsgUtils.queryAll(kerberosJson, 'tagLabel', 'GeneralString');
    ...
    it('test kerberos AP-REP msg', () => {
        const output = ASN1Node.parseFileMsg(resolve(__dirname, 'data/kerberos/krb-ap-rep.base64.der'), {
            encodingRule: EncodingRule.DER,
    ...
    it('validate AP-REP', () => {
        ...
        const module = ASN1.getModuleFromStr(definition);
        const validated = ASN1.validate(module, krbApRepJson, 'GSS-API');
        // console.log('validated: ', inspect(validated, false, null, true));
